...
void handler_shutdown(int signal){
	...
	set_color('r', 0);
	clear_screen();
//	printf("\n----------[CLIENT_2] Dostal som signál %d. Vypínam sa...----------\n", signal);
...
void* server_comm(){
	...
	while(1){																				//komunikacia so serverom
		...
		if(connect(cl_sock, (struct sockaddr*)&server_addr, sizeof(server_addr)) == -1){		//pripojenie na server
			perror("!!![CLIENT_2] Pripojenie na server zlyhalo");								//
			close(cl_sock);																		//
			sleep(2);																			//
			continue;																			//
		}																						//
		...
		if(items != 4)	printf("!!![CLIENT_2] Chyba čítania prijatých dát\n");
